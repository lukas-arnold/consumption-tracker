...
func TestAddOilPost(t *testing.T) {
	...
	form.Set("date", "2024-01-01")
	form.Set("volume", "1000")
	form.Set("costs", "900")
	form.Set("retailer", "Shell")
...
func TestSaveOilFillLevel(t *testing.T) {
	...
	if updated[0].Level != 100 {
		t.Fatal(
			"fill level not updated",
		)
	}

	if updated[0].Date != "2025" {
		t.Fatal(
			"date not updated",
		)
	}
