...
func formatPrice(costs float64, units float64, unit string) string {
	if units <= 0 {
		return "-"
	}
...
func calculateMonthlyPeriod(payments float64, days float64) float64 {
	return payments / days * 30.4375
}

func formatMonthly(payments float64, timeFrom string, timeTo string) string {
	if payments == 0 {
		return "-"
	}
	...
	from, errFrom := utils.ConvertTime(timeFrom)
	to, errTo := utils.ConvertTime(timeTo)
...
func formatDifference(payments float64, costs float64) string {
