...
func (s *Storage) AddOilFillLevel(
	oilFillLevel models.OilFillLevelInput,
) error {
...
func (s *Storage) GetOilFillLevels() ([]models.OilFillLevel, error) {
	...
	for i := range storage.OilFillLevels {
		...
		storage.OilFillLevels[i].Percentage =
			calculateOilFillPercentage(
...
func (s *Storage) UpdateOilFillLevel(
...
func (s *Storage) DeleteOilFillLevel(id int64) error {
	...
	for i := range storage.OilFillLevels {
		...
		if storage.OilFillLevels[i].Id == id {
			...
			storage.OilFillLevels = slices.Delete(
				storage.OilFillLevels,
				i,
				i+1,
			)
